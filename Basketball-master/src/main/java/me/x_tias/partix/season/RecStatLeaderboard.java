...
/**
 * Handles Rec Center stat leaderboards
 * Format: [RANK] Username - Stat
 * Example: [ADMIN] posterizing - 500 Rec Wins
 */
public class RecStatLeaderboard {
    ...
    // Rec stats to track
    private static final PlayerDb.Stat[] REC_STATS = {
        PlayerDb.Stat.REC_WINS,
        PlayerDb.Stat.REC_GAMES
    };
    
    public static void setup() {
        // Initialize leaderboard maps for each stat
        for (PlayerDb.Stat stat : REC_STATS) {
    ...
    private static String getStatDisplayName(PlayerDb.Stat stat) {
        return switch (stat) {
            case REC_WINS -> "Rec Wins";
            case REC_GAMES -> "Rec Games";
            default -> "Stats";
        };
    }
    ...
    public static String getLeaderboardTitle(PlayerDb.Stat stat) {
        return switch (stat) {
            case REC_WINS -> "§6§lRec Center Wins Leaderboard";
            case REC_GAMES -> "§6§lRec Center Games Leaderboard";
            default -> "§6§lRec Center Leaderboard";
        };
    }
